...
static size_t ref_popcount(const PllSplit &split, size_t len) {
    ...
    for (size_t index = 0; index < len * PllSplit::splitBitWidth(); ++index) {
        if (split.bit_extract(index) == 1) { popcount += 1; }
    }
...
TEST_CASE("popcount", "[primitives]") {
    auto split_lists = Util::create_splits(20, "125");
    unsigned int tip_count =
        static_cast<unsigned int>(split_lists[0].size()) + 3;
    unsigned int tip_ints = (tip_count + 31) / 32;

    std::cout << "tip_count: " << tip_count << std::endl
              << "tip_ints: " << tip_ints << std::endl;

    std::vector<PllSplit> splits;
    for (const auto &s : split_lists) {
        for (size_t i = 0; i < s.size(); i++) { splits.emplace_back(s[i]); }
    }
